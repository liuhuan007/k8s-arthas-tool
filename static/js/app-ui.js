...
// 每个连接的采样日志缓存
let _pfLogsByConn = {}; // { connId: [{message, level}, ...] }
// 每个连接的采样任务状态缓存
let _pfTasksByConn = {}; // { connId: { taskId, pollTimer, startTime, duration, logLines, status } }
...
async function switchConnection(connId) {
  ...
  try {
    ...
    if (d.ok) {
      ...
      // 保存当前连接的采样任务状态
      if (_pfTaskId || _pfPollTimer) {
        _pfTasksByConn[_currentConnId] = {
          taskId: _pfTaskId,
      ...
      if (targetTask && targetTask.taskId && targetTask.status === 'running') {
...
function pfLog(msg, lv='info') {
  ...
  // 保存到当前连接的日志缓存（保存原始消息，包含换行）
  if (_currentConnId) {
    if (!_pfLogsByConn[_currentConnId]) {
      _pfLogsByConn[_currentConnId] = [];
    }
    _pfLogsByConn[_currentConnId].push({message: msg, level: lv});
  }
}
function pfClearLog() {
  const el = document.getElementById('pfl-panel-log');
  ...
  // 清空当前连接的日志缓存
  if (_currentConnId && _pfLogsByConn[_currentConnId]) {
    _pfLogsByConn[_currentConnId] = [];
  }
  ...
  try {
    ...
    el.innerHTML = currentTasks.map(t => `
      <div style="padding:10px;background:var(--bg1);border:1px solid var(--ln);border-radius:6px;margin-bottom:8px">
        ...
        ${t.has_file?`<a href="${API}/profile/${t.id}/download" class="btn btn-dl" style="display:inline-block;margin-top:6px;padding:4px 10px;font-size:10px" download>↓ 下载文件</a>`:''}
      </div>
    `).join('');
  } catch(e) {
...
function loadConnectionProfilerLogs(connId) {
  // 加载连接的采样日志
  const el = document.getElementById('pfl-panel-log');
  if (!el) return;

  const logs = _pfLogsByConn[connId] || [];
  if (logs.length === 0) {
