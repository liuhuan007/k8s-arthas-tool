...
```json
{
  ...
  "error": {
    "code": 2001,
    "message": "诊断能力不存在",
    "detail": "capability_id=999 not found"
  }
}
```
...
| 范围 | 类别 | 说明 |
|------|------|------|
| 1xxx | 认证/授权错误 | 登录、权限、Token |
| 2xxx | 业务逻辑错误 | 数据不存在、状态冲突、参数校验 |
| 3xxx | 外部依赖错误 | K8s API、kubectl、Arthas |
| 4xxx | 系统内部错误 | 数据库、文件系统、网络 |
...
| 1002 | 403 | 无权限访问该资源 |
| 1003 | 401 | 用户名或密码错误 |
| 1004 | 403 | 非管理员无权访问管理接口 |

#### 2xxx 业务逻辑

| 错误码 | HTTP | 说明 |
|--------|------|------|
| 2001 | 404 | 资源不存在（连接/能力/任务） |
| 2002 | 400 | 参数校验失败 |
| 2003 | 409 | 状态冲突（如连接未就绪时执行诊断） |
| 2004 | 400 | 重复操作（如连接已存在） |
| 2005 | 404 | Pod 不存在或不可达 |
| 2006 | 400 | Arthas 未启动或未就绪 |
| 2007 | 400 | 诊断能力参数错误 |
| 2008 | 409 | 任务正在执行中，无法重复提交 |

#### 3xxx 外部依赖

| 错误码 | HTTP | 说明 |
|--------|------|------|
| 3001 | 502 | K8s API 调用失败 |
| 3002 | 502 | kubectl 执行失败 |
| 3003 | 502 | Arthas HTTP 调用失败 |
| 3004 | 504 | 外部依赖超时 |
| 3005 | 502 | LLM API 调用失败 |
