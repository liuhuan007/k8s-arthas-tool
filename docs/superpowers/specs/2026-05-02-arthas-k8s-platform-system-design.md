...
- **统一错误响应格式**: 所有 API 错误使用标准化 JSON 格式
- **错误分类清晰**: 通过错误码区分认证、连接、执行、资源等错误类型
- **开发/生产区分**: 开发环境提供详细堆栈，生产环境隐藏敏感信息
- **审计友好**: 所有异常自动记录日志，便于问题追踪
...
|----------|------|------|
| 0 | 成功 | `SUCCESS` |
| 1xxx | 通用错误 | `UNKNOWN_ERROR`, `INVALID_REQUEST` |
| 2xxx | 认证错误 | `AUTH_REQUIRED`, `AUTH_FAILED`, `PERMISSION_DENIED` |
| 3xxx | 连接错误 | `CONNECTION_FAILED`, `POD_NOT_FOUND`, `ARTHAS_NOT_READY` |
| 4xxx | 执行错误 | `COMMAND_FAILED`, `COMMAND_TIMEOUT`, `EXEC_FAILED` |
| 5xxx | 资源错误 | `FILE_NOT_FOUND`, `DISK_FULL` |
| 6xxx | 业务错误 | `INVALID_PARAMS`, `RESOURCE_BUSY` |
...
**处理器列表**:
- `AppException` - 应用自定义异常
- `AuthException` - 认证失败 (401)
- `PermissionException` - 权限拒绝 (403)
...
```python
from backend.exceptions import handle_exceptions
...
```python
from backend.exceptions import validate_request

@app.route('/api/pod/check', methods=['POST'])
...
```python
from backend.exceptions import NotFoundException, ConnectionException
...
def check_pod():
    ...
    # 验证 Pod 是否存在
    if not pod_exists(data['namespace'], data['pod_name']):
        raise NotFoundException(
            message=f"Pod {data['pod_name']} 在 namespace {data['namespace']} 中不存在",
            details={
...
```javascript
const res = await fetch('/api/pod/check', { ... });
...
if (!data.ok) {
  ...
  switch (errorCode) {
    case 2001: // AUTH_FAILED
      window.location.href = '/login.html';
      break;
    case 3001: // POD_NOT_FOUND
      toast(`Pod 不存在: ${message}`, 'error');
      break;
    case 4000: // COMMAND_FAILED
      toast(`命令执行失败: ${message}`, 'error');
      break;
    default:
...
| 异常类型 | 日志级别 | 记录内容 |
|---------|---------|----------|
| `AppException` | WARNING | 错误码、消息、请求路径 |
| `AuthException` | WARNING | 用户 ID、IP、失败原因 |
| `PermissionException` | WARNING | 用户 ID、请求资源、权限 |
| `NotFoundException` | WARNING | 请求路径、资源标识 |
| `BadRequestException` | WARNING | 请求参数、缺失字段 |
| `Exception` (未捕获) | ERROR | 完整堆栈、请求信息 |

### 15.9 前端错误提示组件

**集成点**: `static/js/components/error-notification.js`
...
async function safePost(url, body) {
  ...
  if (!data.ok) {
    // 显示精准错误提示
    showErrorNotification({
      title: getErrorTitle(data.error_code),
      message: data.message,
      suggestion: getSuggestion(data.error_code)
    });
  }
...
1. **优先使用自定义异常**: 不要直接 `raise Exception()`,使用具体异常类
2. **提供详细信息**: `details` 字段包含上下文,便于调试
3. **用户友好消息**: `message` 字段面向用户,避免技术术语
4. **生产环境隐藏堆栈**: 仅在开发环境暴露 `stack_trace`
5. **前端按错误码处理**: 不要依赖 `message` 内容判断错误类型
6. **记录审计日志**: 认证/权限异常自动记录到 `audit_logs` 表
